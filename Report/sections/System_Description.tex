...
This class represents the register checkouts where the cashiers are seated. Every register belongs to three particular lanes in the gasstation. Each checkout can either be in the state \textit{open}, which means the checkout is occupied with a cashier and cars can refuel in the associated lanes, or \textit{closed}. In the last state the checkout can be manned. When it is, the pumps are already closed but all the vechicles which were in lane should be handled before the cashier leaves. 
